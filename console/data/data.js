    ...
    sessions: [
      {
        ...
        segments: [
          ...
          },
          {
            ...
            speakerNotes: "先用新增兩頁，把 AI 的脈絡講成白話：規則系統→機器學習→文字／語音／影像辨識→生成式 AI。再分清辨識、預測、生成、執行四種工作：今天練的是生成初稿，不是把判斷交出去。接著連結既有觀念：AI 生成的是最可能的文字、搜尋是找來源；再帶任務拆解六格：目標、對象、交付物、限制、時程、待確認。",
        ...
        segments: [
          {
            ...
            speakerNotes: "先用新增兩頁說明多模態能力：文字與文件、看與做圖片、聲音／影片／音樂；提醒生成內容不自帶正確性或授權。再用任務挑能力：文字初稿、文件問答、創作素材、多步驟工作分別有不同工具與安全邊界。接著用工具地圖快速定位 ChatGPT、Gemini、Manus，請各組挑一件需要溝通的事。",
        ...
        segments: [
          {
            ...
            speakerNotes: "先用新增兩頁深入 Agent：它接目標、規劃、呼叫工具、回報與等待；權限一開就可能真的造成動作。再用 Klarna 客服與 Waymo 特定範圍營運作為案例，重點是範圍清楚、結果可驗證、有人接手。接著複習：對話式 AI 適合草稿、帶來源 AI 適合讀資料、代理型 AI 適合可撤回的多步驟公開／模擬任務。再用混亂會議筆記問大家：哪些只是討論、哪些已定案、哪些沒有主責？",
      ...
      },
      {
        ...
        segments: [
          ...
          },
          {
            ...
            speakerNotes: "每組一分鐘，依『資料正確、訊息清楚、下一步可執行』互評。實作後用新增四頁帶回 AI 資安：資料／提示語／權限／結果四個出口、間接 Prompt Injection、資料外洩常見路徑，以及 EchoLeak、DeepSeek 的風險案例。講清楚案例是提醒，不是說某工具必然不能用；最後回到三條工作紅線：安全、正確、負責。請學員選一件非敏感的真實工作，未來一週照今天流程跑一次，並掃課後問卷 QR Code。",
